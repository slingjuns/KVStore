	...
	for i := 0; i < npara; i++ {
		...
		go func(me int) {
			...
			for iters := 0; iters < 3; iters++ {
				...
				log.Printf("Appending Key:  %v, Value: %v ", key, nv)
				ck.Append(key, nv)
				log.Printf("Append Key:  %v, Value %v done!", key, nv)
				last = last + nv
				log.Printf("Getting %v, Key: %v", nv, key)
				v := ck.Get(key)
				log.Printf("Get %v done! Key:  %v, Value: %v !", nv, key, v)
				if v != last {
					ok = false
				...
				log.Printf("Moving to %v!", gid)
				mymck.Move(rr.Int()%common.NShards, gid)
				log.Printf("Move to %v done!", gid)
			}
		}(i)
	}
...
func TestUnreliableConcurrent(t *testing.T) {
...
func TestLinear(t *testing.T) {
	fmt.Printf("Test: Linearizablity\n")
	...
	for i := 0; i < npara; i++ {
		ca[i] = make(chan bool)
		go func(me int) { // concurrent (multiple clients)
			ok := true
			defer func() { ca[me] <- ok }()
			ck := tc.clerk()
			// mymck := tc.shardclerk()
		...
		go func(me int) {
			sck := tc.shardclerk()
			nreplicas := 3
			var j int = me*nreplicas + 1000
			newGroup := &tGroup{}
			newGroup.gid = int64(j)
			newGroup.servers = make([]*ShardKV, nreplicas)
			newGroup.ports = make([]string, nreplicas)
			for k := 0; k < nreplicas; k++ {
				newGroup.ports[k] = port("linear"+"s", j+k)
			}
			for k := 0; k < nreplicas; k++ {
				s := StartServer(int64(j), newGroup.ports, k)
				newGroup.servers[k] = s
			}
			sck.Join(newGroup.gid, newGroup.ports)
		}(i)
		go func(me int) {
			sck := tc.shardclerk()
			var k int64
